...
rank_and_nb_group_mig<-function(df, var, order="Descending", peers="Current",
  ...
  p <- p + theme(text = element_text(family = "Museo Sans 300"),
                 ...
                 plot.caption = element_text(),
                 plot.subtitle = element_text(hjust = 0.5))
  p <- p+geom_text(aes(label=round),hjust=ifelse(d.graph$var>1000, 1.1, ifelse(d.graph$var>0,.2,ifelse(d.graph$var>-1000, 1.0,-.1))), size=5, family = "Museo Sans 300")
  p <- p+labs(title = plot_title, y= y_title,
              x = "", caption = caption_text)
  p
}
...
jpeg("volunteer_rate_rankings.jpg", 900, 600, res = 100)
showtext.begin()
rank_and_nb_group(
