...
require("knitr")
opts_knit$set(root.dir = "~/Desktop/glp_website/input data/qp_input_data")
```
...
cbp_read<-function(folder, starting_year){
  ...
  y <- starting_year
  for(i in 1:n){
    data<-read_csv(paste("/Users/BryceRowland/Desktop/glp_website/input data/qp_input_data",folder,file_names[i], sep = "/"), skip = 0, progress = FALSE)
...
house_dat <- read_csv("all_transactions_house_price_index.csv", skip = 1, progress = FALSE)
burdened_dat <- read_csv("burdened_households.csv", skip = 1, progress = FALSE)
disconnected_dat <- read_csv("disconnected youth.csv", skip = 1, progress = FALSE)
home_own_dat <- read_csv("homeownership_rate.csv", skip = 1, progress = FALSE)
income_inequality_dat <- read_csv("income_inequality.csv", skip = 1, progress = FALSE)
commute_time_dat <- read_csv("mean_commuting_time.csv", skip = 1, progress = FALSE)
net_migration_flow_dat <- read_csv("net_migration_flow.csv", skip = 1, progress = FALSE)
race_dat <- read_csv("white to nonwhite racial dissimilarity index.csv", skip = 1, progress = FALSE)
...
names <- read_csv("FIPS_two_stl.csv", col_types = cols(FIPS = col_integer()), progress = FALSE)
...
dat$FIPS[dat$city == "St. Louis"] <- "0"
dat$FIPS <- as.factor(dat$FIPS)

population_one_stl<-read_csv("population_data_one_stl.csv", progress = FALSE)
...
sc_data_5_6 <- cbp_read("Social Support 5_6", starting_year = 2005)
sc_data_5_6 %<>% select(naics, FIPS, year, baseline, current, est)

sc_data_7_14 <- cbp_read("Social Support 7_14", starting_year = 2007)
sc_data_7_14 %<>% select(naics, FIPS, year, baseline, current, est)

sc_data_15 <- read_csv("cbp15co.txt.zip", progress = FALSE)
...
```{r}
#Read in data
fips_core_county_pop<-read_csv("population_data_one_stl.csv", progress = FALSE)

#Create "key" for core FIPS county to MSA
msa_fips_key<-read_csv("changed_msa_counties.csv", progress = FALSE)
...
msa_pop_est<-read_csv("cbsa-est2016-alldata.csv", n_max = 1581, progress = FALSE)
...
```{r}
#Read in data
food_insec<-read_csv("GLP counties food insecure_PCT.csv", progress = FALSE)
...
weights <-
  read_csv(
  "~/Desktop/glp_website/input data/qp_input_data/FIPS_two_stl.csv",
...
```{r}
#Read in Data
fd_data <- read_csv("food_desert_2015.csv",
...
fd_data$FIPS <- as.character(fd_data$FIPS)
...
```{r}
#Reading in Data
chr_df_17 <- read_csv("2017CHR_CSV_Analytic_Data.csv",
                       col_types = cols(
...
name_df <- read_csv("FIPS two stl.csv")
...
chr_stl$FIPS[chr_stl$city == "St. Louis"] <- "0"
...
```{r}
#Reading in Data
pc_ed_data<-read_csv("ACS_14_5YR_B23006_with_ann.csv", skip = 1)
...
pc_health_data<-read_csv("ACS_14_5YR_S2701_with_ann.csv", skip=1)
...
pc_low_inc_data<-read_csv("ACS_14_5YR_C17002_with_ann.csv", skip=1)
pc_low_inc_data%<>%
  select(
    Id2,
...
pc_unemp_data<-read_csv("ACS_14_5YR_S2301_with_ann.csv",skip=1)
pc_unemp_data%<>%
  select(Id2, 
...
v_2015 <- read_csv("~/Desktop/glp_website/input data/qp_input_data/volunteer2015.csv", progress = FALSE)
