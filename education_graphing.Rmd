...
educ_data[ ,c(3:17)] = apply(educ_data[ ,c(3:17)], 2, rescale)
educ_data_15 <- educ_data %>% filter(year == 2015)


naep_data <- read_csv("output data/education_data_naep.csv")
act_ccr_data <- read_csv("output data/education_data_act_ccr.csv")

#Plot ACT by Ethnicity for JCPS
jeff_eth_act <- act_ccr_data %>%
...
if(!(exists("rank_and_nb_group", mode = "function") &
     ...
     exists("ky_ed_data_long_trendline", mode = "function") & 
     exists("graph_trendline_ky_ed")
...
graph_trendline(educ_data, "under_5_per",
                  ...
                  subtitle = "3-year rolling average", rollmean = 3, xmin = 2006, 
                  xmax = 2014,
                  caption_text = "Source: Greater Louisville Project \nData from the American Community Survey, Table B17001")
showtext.end()
dev.off()
...
graph_trendline(educ_data, "bach_plus_per_all",
                  ...
                  rollmean = 5, xmin = 2007, xmax = 2013,
                  caption_text = "Source: Greater Louisville Project \nData from the American Community Survey, Table B15002")
showtext.end()
dev.off()
...
Adding mapping data
```{r}
map_data = read_csv("output data/education_map_data.csv", col_types = cols(enrolled_3_4 = col_double()))

rescale <- function(x){new_value <- x*100}
map_data[ ,c(3:5, 7:14)] = apply(map_data[ ,c(3:5, 7:14)], 2, rescale)
...
m <- make_map("child_per", "Child Poverty", order = "Descending", col_palette = "RdYlBu")
